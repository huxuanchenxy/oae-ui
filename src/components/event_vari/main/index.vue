...
function getEventAndParamMaxNum(cfg){
  ...
  let eventMaxNum=eventInputNum>eventOutputNum?eventInputNum:eventOutputNum;//事件数量取最大值，用于计算需要把整个框撑起多大
  ...
  let paramMaxNum=paramInputNum>paramOutputNum?paramInputNum:paramOutputNum;//参数数量取最大值，用于计算需要把整个框撑起多大
  if(eventMaxNum<=5){
    eventMaxNum=5;
  }
  if(paramMaxNum<=5){
  ...
  */
  draw(cfg, group) {
    ...
    cfg.paramInput?.forEach((element,index) => {
      ...
      const paramInputHeight=startPosition+index*blockSide*2+blockSide
      group.addShape('rect',  {
        attrs: {
          width:blockSide,
          height:blockSide,
    ...
    cfg.paramOutput?.forEach((element,index) => {
      const startPosition=height/2-paramMaxNum*blockSide*2//从最下面开始减去所有的参数块就是开始位置 
      const paramOutputHeight=startPosition+index*blockSide*2+blockSide
      group.addShape('rect',  {
        attrs: {
          width:blockSide,
          height:blockSide,
  ...
  getPath(cfg) {
    ...
    const path = [
      ...
      ['L',width / 2-blockSide*2,-height / 2+2*eventMaxNum*blockSide], 
      ['L',width / 2-blockSide*2,-height / 2+2*eventMaxNum*blockSide+2*blockSide], 
      ['L',width / 2,-height / 2+2*eventMaxNum*blockSide+2*blockSide], 
      ['L',width / 2,height / 2], 
      ['L',-width / 2,height / 2], 
      ['L',-width / 2,-height / 2+2*eventMaxNum*blockSide+2*blockSide], 
      ['L',-width / 2+blockSide*2,-height / 2+2*eventMaxNum*blockSide+2*blockSide], 
...
let data = {
  nodes: [
    {
      id: 'node1',  x: 150, y: 200,type: 'myShape' ,color: 'red'
